...
VENDORED COPY of src/api/lz/calculator_export.py — `ca-calc` builds its own
`src/calc` image (separate build context) so it can't import from `src/api`. Keep
the two in sync; both are covered by tests (tests/test_calculator_export.py and
...
def parse_calculator_export(source) -> dict:
    ...
    estimate_name = _s(cell(1, 0)) or _s(title)

    hdr = next((i for i, r in enumerate(rows) if _s(r[0]).lower() == _HEADER_A), 2)
