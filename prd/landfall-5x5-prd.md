...
- **E5.4** **Client-ready exports — Excel, Word, PowerPoint.** The assembled package is
  ...
  - **PowerPoint deck (`.pptx`)** — the executive readout: title slide, headline
    numbers, one slide per major section (current state, landing zone, disposition +
    waves, run-rate cost, effort), an assumptions slide. This in-Function deck is the
    *always-available baseline*; the client-facing deck is produced by the studio path
    in **E5.6**.
...
- **E5.6** **Studio-grade client deck (`ppt-master` + `presentation-skill`).** The E5.4
  ...
  of a client is generated by the two open skills the sponsor selected:
  ...
  - **Where it runs.** Both skills need Node + pptxgenjs + (optionally) LibreOffice /
    Poppler for render verification — not available in the Function runtime. So E5.6 is a
    **side-car**: either (a) an architect runs the skill locally against the downloaded
    `latest.json`, or (b) a dedicated build container the dashboard can invoke ("Generate
    studio deck") that carries the Node toolchain. The Function's E5.4 `.pptx` stays the
