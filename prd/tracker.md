...
_Last updated: 2026-09-08 (PDCA cycles 1–17). C15 = first live deploy to `rg-landfall`/swedencentral (E1.1 / E1.5 / E1.6 / E5.4 / E5.5 verified; 3 deploy bugs fixed). C16 = **E8.2** Function App EasyAuth on (anon `/api/*` → 401, agent via MSI, ingestion intact). C17 = **E5.4 `.pptx` rebuilt** as a 12-slide narrative assessment deck (native charts, MEG lifecycle, watermark every slide), deployed + published live. Phase 1 engine complete bar E1.7 / E4.3 / full E6.2. Generation model: `export.py` deterministic Python via `export_estimate`/`publish_estimate` — **the Foundry agent orchestrates, it does not author documents**; the 4 external skills are design references (docx/xlsx) or a Phase-2 side-car (presentation-skill/ppt-master). 127 pytest + 32 golden SQL + 8 scenarios + 30 fault cases green. Known gap: `schema.sql` drops+recreates tables every `azd provision` (wipes inventory) — needs idempotent migrations._
...
| ID | Item | Pri | Status | Owner | Cycle | Acceptance |
|---|---|---|---|---|---|---|
| E11.1 | Engagement model + provisioning (slug rules, `_engagement.json`, `POST/GET /api/engagements`) | P0 | in-review | App+Data | 18 | Creating "Contoso / DC-Exit" makes the folder skeleton + returns the id; duplicate → `-2`. _`src/api/engagement.py` + `engagements.py` blueprint; 22 tests._ |
| E11.2 | Per-engagement ADLS layout (`raw/engagements/<c>/<p>/…`, `answers/engagements/…`); Event Grid subject filter + blob-trigger path | P0 | in-review | Data Eng | 18 | A file under one engagement never triggers ingestion for another. _Blob trigger `raw/engagements/{customer}/{project}/inventory/{name}`; `eventgrid.{sh,ps1}` subject → `/blobs/engagements/`; DQ reports under `answers/engagements/<c>/<p>/_ingest/`._ |
| E11.3 | SQL tenancy — `engagement_id` on all 6 tables + **Row-Level Security** (filter predicate + `SECURITY POLICY`, `sp_set_session_context`), composite PKs, loader keying | P0 | in-review | Data Eng | 18 | Two engagements' rows coexist; with no session context set, every table returns nothing. _`schema.sql` + `engagement_sql.py`; `loader.load(table, rows, engagement)`._ |
