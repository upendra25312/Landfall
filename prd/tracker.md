...
| E10.4 | `evidence/{pentest,trials,chaos}/` reports | P0/P1 | in-progress | Security + SRE + Writer | 36,38 | Pen test passes; usability + comprehension trials meet bars; chaos drill logged. _`evidence/trials/` kit built (C36) — both trials turnkey, comprehension apparatus dry-run-validated. `evidence/chaos/` (C38) — 6 failure modes + `probe.py` (7/7 mitigations in place), SQL auto-pause induced+observed, ca-drawio cold-start verified live, C3–C6 by inspection. **Still:** run the 2 human trials; `pentest/` not started; induce C2–C6 end-to-end on a scratch env._ |
