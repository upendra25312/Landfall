...
**NEEDS azd deploy web**, bundled with C48. E13.6 in-review pending live acceptance.
**Epic E13: 8/17 done, 1 in review. Next:** E13.5 (agent-run ceiling) / E13.9 (E12 tail).
...
| **E13.6** | **Split `src/web/app.py`** into `APIRouter` modules (`routes/chat.py`, `routes/engagements.py`, `routes/dashboard.py`, `routes/questionnaire.py`, plus uploads, analysis, transfers, pages). Shared helpers: `web_runtime.py`, `web_storage.py`, `web_access.py`, `chat_state.py`; routers never import the entrypoint. | P2 | in-review (C52 code complete) | `app.py` 54 lines; routers 68–185 lines. C51 OpenAPI contract + all 30 route/method combinations preserved; tests/evals/browser checks logged in C52, before/after desktop/mobile screenshots identical. **NEEDS azd deploy web** by operator, bundled with C48; live acceptance pending. |
...
Before/after desktop and mobile screenshots are identical. Code complete;
**NEEDS azd deploy web** by the operator, including the C48 chat fix.
