...
- **`ca-calc` scales to zero (C25b):** `minReplicas 0`, a KEDA `azure-queue` scale rule on
  `calc-jobs` (workload-identity auth, no account key), `cooldownPeriod 900 s` so a replica
  is never scaled out from under an in-flight calculator drive; a job killed anyway
  reappears after the 1800 s queue visibility timeout and retries (worker `_MAX_DEQUEUE 3`).
