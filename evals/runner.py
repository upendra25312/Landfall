...
def scorecard(golden, scenarios, faults=None) -> str:
    ...
    L = [f"# Landfall eval scorecard", "",
         f"_Regenerated by the offline harness: `python evals/runner.py`. "
         f"Committed; CI fails if it drifts._", "",
         f"| Suite | Result | Gate |", "|---|---|---|",
    ...
    if faults is not None:
        L.append(f"| Fault injection (E7.3) | {faults['passed']}/{faults['total']} | "
                 f"all pass -> {ok(f_ok)} |")
    L += [f"| **Overall** | **{ok(g_ok and s_ok and f_ok)}** | |", "",
...
def main():
    ...
    card = scorecard(golden, scenarios, faults)
