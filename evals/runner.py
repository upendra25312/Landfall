...
def scorecard(golden, scenarios) -> str:
    now = _dt.date.today().isoformat()
    g_ok = golden["rate"] >= GOLDEN_GATE
    s_ok = scenarios["passed"] == scenarios["total"]
    ok = lambda b: "PASS" if b else "FAIL"
    L = [f"# Landfall eval scorecard", "",
         f"_Generated {now} - offline harness (`python evals/runner.py`)_", "",
         f"| Suite | Result | Gate |", "|---|---|---|",
         f"| Golden text-to-SQL (E7.1) | {golden['passed']}/{golden['total']} "
         f"({golden['rate'] * 100:.0f}%) | >={GOLDEN_GATE * 100:.0f}% -> {ok(g_ok)} |",
         f"| Full-estimate scenarios (E7.2) | {scenarios['passed']}/{scenarios['total']} | "
         f"all pass -> {ok(s_ok)} |",
         f"| **Overall** | **{ok(g_ok and s_ok)}** | |", "",
...
def main():
    ...
    golden = run_golden(verbose=not args.quiet)
    if not args.quiet:
        print(f"  {golden['passed']}/{golden['total']} ({golden['rate'] * 100:.0f}%)\n")
        print("Full-estimate scenarios (E7.2):")
    scenarios = run_scenarios(verbose=not args.quiet)
    if not args.quiet:
        print(f"  {scenarios['passed']}/{scenarios['total']}\n")

    card = scorecard(golden, scenarios)
